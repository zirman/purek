package functional.parser.array
...
typealias ParserIn<Token> = List<Token>
typealias ParserOut<Token, A> = List<Pair<List<Token>, A>>
...
fun <T, A> A.toParser(): Parser<T, A> = { ts -> listOf(Pair(ts, this)) }
...
fun <T, A> Parser<T, A>.and(p: Parser<T, A>): Parser<T, A> =
    { ts -> listOf(invoke(ts), p(ts)).flatten() }
...
inline fun <T, reified A : Any> match(): Parser<T, A> =
    { ts ->
        ts
            ...
            ?.let { x -> listOf(Pair(ts.drop(1), x)) }
            ?: emptyList()
    }

fun <T> matchNothing(): Parser<T, Unit> =
    { ts -> listOf(Pair(ts, Unit)) }
